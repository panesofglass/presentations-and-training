...
class DatabaseConnectionReader(IFileFormatReader):
  """
  Determines whether the log is from a database.

  The getmessagebody method does nothing. This is unexpected
  and is the reason for the LSP violation.
  http://www.themacaque.com/?p=155
  """

  def canhandle(self, filecontents):
    """
    Determines whether the reader can handle the file contents.

    >>> DatabaseConnectionReader().canhandle('blah blah blah')
    False
    >>> DatabaseConnectionReader().canhandle('server=myserver')
    True
    """
  ...
  def getmessagebody(self, filecontents):
    """
    >>> DatabaseConnectionReader().getmessagebody('server=myserver')
    Traceback (most recent call last):
	...
    NotImplementedError: Need to read from the database!
    """
    raise NotImplementedError('Need to read from the database!')
...
class FileReaderService(object):
  ...
  def getmessagebody(self, filecontents):
    ...
    >>> service.register(DatabaseConnectionReader())
    >>> service.register(XmlFileFormatReader())
    >>> service.register(FlatFileFormatReader())
    >>> service.getmessagebody("server=myserver")
    'Hello world!'
	...
	if isinstance(r, DatabaseConnectionReader):
	  messagebody = "Hello world!" # Get the log data from the database.
	else:
...
class EmailSender(object):
  def __init__(self):
    ...
    self._fileReaderService.register(DatabaseConnectionReader())
